/*
* Create an ELF object . with 2 section, data and strtab only
*/
...
char string_table [] = { 
/* Offset 0 */ '\0' ,
/* Offset 1 */ '.' , 'f' , 'o', 'o' , '\0' , // .foo
/* Offset 6 */ '.' , 's' , 'h' , 's' , 't' ,'r' , 't' , 'a' , 'b' , '\0' //.shstrtab
};
...
int main(int argc, char **argv){
    ...
    //elf_setshstrndx(e, elf_ndxscn(scn)); //my lib dont have this api so i will use alternative solution
    ehdr->e_shstrndx = elf_ndxscn(scn);
    
    if(elf_update(e, ELF_C_NULL) < 0)
        errx(EX_SOFTWARE, "elf_update(NULL) failes %s", elf_errmsg(-1));
    ...
    if(elf_update(e, ELF_C_WRITE) < 0)
        errx(EX_SOFTWARE, "elf_update() failes %s", elf_errmsg(-1));
